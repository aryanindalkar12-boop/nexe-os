...
const logoMap: Record<string, string> = {
  ...
  Premiere: "/icons/integrations/pr.svg",
  Docs: "/icons/integrations/docs.svg",
  Drive: "/icons/integrations/drive.svg",
  AfterFx: "/icons/integrations/ae.svg",
  Illustrator: "/icons/integrations/ils.svg",
};
...
const IntegrationCard = ({
  ...
  return (
    <div
      className={cn(
        "relative group flex rounded-full",
        className
      )}
    >
      {iconSrc ? (
      ...
      <Wrapper>
        <div className="aspect-16/10 group relative mx-auto flex max-w-80 items-center justify-between sm:max-w-xs lg:max-w-md">
          ...
          <Container
            animation="blurIn"
            delay={0.5}
            className="absolute inset-x-0 bottom-0 lg:bottom-1/10 mx-auto my-2 flex justify-center gap-2 w-fit scale-90 lg:scale-100"
          >
            <div className="bg-primary/5 relative z-20 rounded-full border border-foreground/10 backdrop-blur-sm p-1.5 group">
              <IntegrationCard className="size-20 relative bg-primary/5">
                {/* <Icons.icon className="text-primary group-hover:scale-110 group-active:scale-95 transition-all duration-300" /> */}
                <Image
                  src="/icons/icon.png"
                  alt="Nexe Logo"
                  fill
                  className="object-contain rounded-full text-primary group-hover:scale-110 group-active:scale-95 transition-all duration-300"
                />
              </IntegrationCard>
            </div>
          </Container>
        ...
        <div className="relative z-20 mx-auto -mt-4 max-w-lg space-y-6 text-center">
          ...
          >
            <Link href={Routes.Dashboard}>
              <Button variant="ios">Get Started</Button>
            </Link>
          </motion.div>
        </div>
      </Wrapper>
